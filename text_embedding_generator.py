...
class LocalTextEmbedder:
    def __init__(self, model_name='all-MiniLM-L6-v2', chunk_size=300):
    ...
    def process_file(self, extracted_text_file):
        if not os.path.exists(extracted_text_file):
            ...
            print(" Please check the path or move the file to the same directory as this script.")
            return
        ...
        for line in text_lines:
            if len(line) > self.chunk_size:
                small_chunks = self._split_into_chunks(line, self.chunk_size)
                text_chunks.extend(small_chunks)
                print(f"  Split long line into {len(small_chunks)} chunks.")
            else:
...
if __name__ == "__main__":
    ...
    embedder = LocalTextEmbedder(chunk_size=300)
