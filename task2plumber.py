...
def extract_text_from_pdf(file_path, use_plumber=False):
...
def extract_text(file_path, output_format="txt", for_tables=False):
    """Main extraction function that detects file type"""
    ext = os.path.splitext(file_path)[1].lower()

    if ext == ".pdf":
        text = extract_text_from_pdf(file_path, use_plumber=for_tables)
    elif ext == ".docx":
...
if __name__ == "__main__":
    ...
    extract_text(file_path, output_format, for_tables)
